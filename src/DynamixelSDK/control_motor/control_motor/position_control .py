...
from dynamixel_sdk_custom_interfaces.msg import SetPositionDual
...
class MotorController(Node):
    def __init__(self):
        ...
        self.position_subscriber = self.create_subscription(SetPositionDual, '/motor_dual_position', self.position_callback, 10)
    ...
    def position_callback(self, msg):
        position1 = msg.motorposition1
        # position2 = msg.motorposition2

        dxl_comm_result1 = self.packetHandler.write4ByteTxRx(
            self.portHandler, DXL_ID1, ADDR_GOAL_POSITION, int(position1)
        )

        if dxl_comm_result1 != COMM_SUCCESS:
            rclpy.logging.get_logger('dynamixel_controller').error(self.packetHandler.getTxRxResult(dxl_comm_result1))
        else:
            rclpy.logging.get_logger('dynamixel_controller').info("Set [ID: %d] [Goal Position: %d]" % (DXL_ID1, position1))
